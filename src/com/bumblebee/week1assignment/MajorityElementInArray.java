package com.bumblebee.week1assignment;

import org.junit.Assert;
import org.junit.Test;

import java.util.Arrays;
import java.util.HashMap;
import java.util.Map;

/*

Question : Given an array nums of size n, return the majority element.
           You may assume that the majority element always exists in the array.
           The majority element is the element that appears more than ⌊n / 2⌋ times.

    1) Did I understand the problem? Yes or No !!
        *         -> If No, Ask the person to provide more detail with example(s)
                      So from question there will always a majority element in the input
        *         -> If yes, go to next step !!
        --------Set of Questions to be asked--------
        *    What is the input(s)? --> In this program the input will be a integer array
        *    What is the expected output? --> Integer that occur more than n/2 times in the array (majority element)
        *    Do I have constraints to solve the problem? --> Ask for constraints (If need to follow specific
                1. Algorithm
                2. Time complexity
                3. Space complexity
        *    Do I have all information to go to next step!!
        *    How big is your test data set will be? --> Ask for the test data(input) size to select the appropriate solution
        (Eg., size of the array)
    2) Test data set
        *    Prepare the positive, negative and edge cases
        *    In this problem
        -----> Positive Cases - {3,2,2,2,4,2} -> Majority element 2
        -----> Negative Cases - {3,4,5,6,7,2} -> No majority element
        -----> Edge Cases -

    3) Do I know how to solve it?
        * Yes - Great, is there an alternate? -> Yes
        * No - Can I break down the problem into sub problems?
        *

    4) Ask for hint (if you do not know how to solve it?)
        *  		-> Ask one time and thank the interviewer
        *  		-> Worst Case! even after got the hint, we don't know how to solve, then convince them and prove with your confidence
        *  		   level to get the next question
        *

    5) Do I know alternate solutions as well?
        * 	Yes - What are those?
        *   Way of Approach to solve with the following
        *   -> Brute Force (Standard) --> using two for loops,
               Collections --> HahMap
        *   Other optimised solutions

    6) If I know the alternate soltions -> find out the 'O' notations.
        --> Start with worst -> Improve -> end up with Best
        --> Write the options and benefits and code the best
    7) Psuedocode
        * Get the input integer array
        * Iterate through the digits using for loops first one to pick element & second to compare the elements
        * Check arr[i] == arr[i+1]
        * If condition is true increment count
        * At end of each iteration check count > arr.length/2, if true then return that element and break the lopp
          else continue with the loop
        *
        */
public class MajorityElementInArray {

    @Test
    public void test1() {
        int input[] = {1,2,3,2,4,2,2};
        Assert.assertEquals(returnMajorityElement(input), 2);
        Assert.assertEquals(returnMajorityElementUsingHashMap(input).intValue(), 2);
    }

    @Test
    public void test2() {
        int input[] = {1,2,3,2,4,2,4};
        Assert.assertEquals(returnMajorityElement(input), -1);
        Assert.assertEquals(returnMajorityElementUsingHashMap(input).intValue(), false);
    }

    private int returnMajorityElement(int input[]) {
        int count = 0;
        int majorityElement = 0;
        int midSize = input.length / 2;
        int i;
        for (i = 0; i < input.length; i++) {
            for (int j = 0; j < input.length; j++) {

                if (input[i] == input[j]) {
                    count++;
                }
            }
            if (count > midSize) {
                majorityElement = input[i];
                break;
            }
            count = 0;
        }
        if(majorityElement!=0)
       return majorityElement;
        else
        return -1;
    }

    private Integer returnMajorityElementUsingHashMap(int input[]) {
        Map<Integer, Integer> map = new HashMap<Integer, Integer>();
        for(int i=0; i<input.length; i++) {
            if(map.containsKey(input[i])) {
                map.put(input[i], map.get(input[i])+1);
            }
            else {
                map.put(input[i], 1);
            }
        }
        return map.entrySet().stream().filter(x -> x.getValue()>(input.length/2)).findFirst().get().getKey();
    }



}
