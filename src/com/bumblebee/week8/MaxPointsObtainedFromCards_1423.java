...
* maximize your total score. The optimal strategy is to take the three cards on the right, giving a final
* score of 1 + 6 + 5 = 12.
Example 2:
...
Output: 202
* */
public class MaxPointsObtainedFromCards_1423 {

    public int maxScore(int[] cardPoints, int k) {
         int min=0;
         for(int i=0;i<cardPoints.length-k;i++)
             min=min+cardPoints[i];
         int sum=min;
         for(int i=1;i<cardPoints.length-(cardPoints.length-k)+1;i++){
             sum=sum-cardPoints[i-1]+cardPoints[i+(cardPoints.length-k)-1];
    ...
    public int maxScore1(int[] cardPoints, int k) {
        int max = 0, winSum = 0;
        for(int i=0; i<k; i++) {
            winSum += cardPoints[i]; // All k elements picked from starting
        }

        max = winSum;
        for(int i=0; i<k; i++) {
            winSum = winSum - cardPoints[k-i-1] + cardPoints[cardPoints.length-i-1];
